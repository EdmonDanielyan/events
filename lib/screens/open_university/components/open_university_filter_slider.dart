...
class _OpenUniversityFilterSliderState extends State<OpenUniversityFilterSlider> {
  ...
  Widget build(BuildContext context) {
    return Container(
        ...
        child: Container(
            child: _getFilterListView(context)
        )
    );
  }
  ...
  List<FilterItem> _getFilterItems() {
    return [
      FilterItem(title: 'О проекте', code: OpenUniversityFilterCodes.about_project),
      FilterItem(title: 'События', code: OpenUniversityFilterCodes.events),
      FilterItem(title: 'ИНК-грамотность', code: OpenUniversityFilterCodes.ink_literacy),
      FilterItem(title: 'Обучающие материалы', code: OpenUniversityFilterCodes.learning_materials)
    ];
  }
}

enum OpenUniversityFilterCodes {about_project, events, ink_literacy, learning_materials}
